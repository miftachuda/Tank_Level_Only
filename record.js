    ...
    ipcRenderer.on('forWin2', function (event, arg) {
        ...
        for (num = 0; num < data.length; num++) {
            if (data[num] != 1) {
                ...
                waktutank.innerText = moment(parseInt(data[num].timestamp)).format("DD-MM-yyyy HH:mm:ss")
                ...
                for (const key in sorted_data) {
                    let newelem = `<div period="${num}" tank="${key}"><span class="namatank">${key}</span><span class="leveltank">${formatNumber(sorted_data[key].level)}</span><span class="temptank" >${sorted_data[key].temp}</span></div>`
